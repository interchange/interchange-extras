...
has _db => (
    is      => 'ro',
    default => sub { my $dsn = Ext::Config->db_dsn; $dsn =~ s/^\w+:\w+:(\w+);.*/$1/; return $dsn; },
);

has _dbh => (
    is      => 'ro',
    isa     => InstanceOf ['DBI::db'],
    default => sub { Ext::Config->dbh },
);

has _db_tables => (
    is      => 'lazy',
    builder => sub { my %tables = map { s/victor//; s/[`.]//g; $_ => 1 } $_[0]->_dbh->tables; return \%tables },
);

has _ic => (
    is      => 'ro',
    isa     => InstanceOf ['Ext::Logger'],
    default => sub {
        Ext::Logger->new({
...
sub _get_variants ($self) {
    return unless $self->_db_tables->{variants};
    my $variants = {};
    my $sql = sprintf(
        q{
            SELECT
    ...
    my $ary = $self->_dbh->selectall_arrayref( $sql, { Slice => {} } );
    for (@$ary) {
