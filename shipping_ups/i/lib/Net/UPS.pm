...
#For international shipments calling one package, sometimes shipment cost is given, but not package cost.

		my $shipment_cost = $services->[0]->total_charges();

		if ($services->[0]->rates()->[0]->total_charges() <= 0.00){
			$services->[0]->rates()->[0]->total_charges( $shipment_cost );
		}
...
sub request_rate {
    ...
    if (my $negotiated_rates = $self->{__args}->{negotiated_rates} ) {
        $data{RatingServiceSelectionRequest}->{Shipment}->{RateInformation}->{NegotiatedRatesIndicator} = $negotiated_rates;
    }
    ...
    my $response    = XMLin( $self->post( $self->rate_proxy, $xml ),
                                            ...
                                            NoAttr => 1,
                                            KeyAttr => [],
                                            ForceArray => ['RatedPackage', 'RatedShipment','RatedShipmentWarning']);
    ...
    for (my $i=0; $i < @{$response->{RatedShipment}}; $i++ ) {
        ...
        $service->negotiated_total_charges( $ref->{NegotiatedRates}->{NetSummaryCharges}->{GrandTotal}->{MonetaryValue} );
        $service->guaranteed_days(ref($ref->{GuaranteedDaysToDelivery}) ?
                                                undef : $ref->{GuaranteedDaysToDelivery});
		if ( $ref->{RatedShipmentWarning} ) {
			my $warnref;
			for (my $sw=0; $sw < @{$ref->{RatedShipmentWarning}}; $sw++ ) {
				$warnref .= "$ref->{RatedShipmentWarning}->[$sw] ";
			}
			$self->set_error( $warnref );
		}
...
#::logDebug("In Net::UPS.pm sending content $content");
    my $response    = $user_agent->request( $request );
#::logDebug("In Net::UPS.pm response:" . $response->content());
#::logDebug("In Net::UPS.pm response:" . $response->status_line());
...
This setting, in conjuction with customer classification setting, and ups account number.
